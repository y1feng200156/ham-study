...
function RadiationPattern() {
  const geometry = useMemo(() => {
    ...
    for (let i = 0; i < posAttribute.count; i++) {
        ...
        const horizontalComponent = Math.sqrt(vertex.x * vertex.x + vertex.z * vertex.z);
...
export default function CircularPolarizationScene({ isThumbnail = false }: { isThumbnail?: boolean }) {
    ...
    const LegendContent = () => (
        <>
            <h2 className="text-lg md:text-xl font-bold mb-2">圆极化 (Circular Polarization)</h2>
        ...
        <div className="flex flex-col gap-4">
            <div className={`relative w-full ${isThumbnail ? 'h-full' : 'h-[450px] md:h-[600px]'} border rounded-lg overflow-hidden bg-black touch-none`}>
                ...
                >
                    ...
                    {showWaves && <RadialWaveLines antennaType="circular" polarizationType="circular" isThumbnail={isThumbnail} />}
                </Canvas>
