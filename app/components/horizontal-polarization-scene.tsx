...
export default function HorizontalPolarizationScene({ isThumbnail = false }: { isThumbnail?: boolean }) {
    const [showWaves, setShowWaves] = useState(true);
    const [showPattern, setShowPattern] = useState(true);

    const LegendContent = () => (
        <>
            <h2 className="text-lg md:text-xl font-bold mb-2">水平极化 (Horizontal Polarization)</h2>
        ...
        <div className="flex flex-col gap-4">
            <div className={`relative w-full ${isThumbnail ? 'h-full' : 'h-[450px] md:h-[600px]'} border rounded-lg overflow-hidden bg-black touch-none`}>
                ...
                >
                    ...
                    {showWaves && <RadialWaveLines antennaType="horizontal" polarizationType="horizontal" isThumbnail={isThumbnail} />}
                </Canvas>
