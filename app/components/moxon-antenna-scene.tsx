...
export default function MoxonAntennaScene({ isThumbnail = false }: { isThumbnail?: boolean }) {
    ...
    return (
        <div className={`relative w-full ${isThumbnail ? 'h-full' : 'h-[450px] md:h-[600px]'} border rounded-lg overflow-hidden bg-black touch-none`}>
            ...
            >
                ...
                {!isThumbnail && <OrbitControls enableDamping dampingFactor={0.05} target={[0, 2, 0]} />}
                ...
                {showWaves && (
                    <group position={[0, 2, 0]}>
                        <RadialWaveLines antennaType="moxon" polarizationType="horizontal" isThumbnail={isThumbnail} />
                    </group>
                )}
            </Canvas>
                ...
                <>
                    <div className="absolute top-4 left-4 right-4 md:right-auto md:w-auto p-3 md:p-4 bg-black/70 text-white rounded-lg max-w-full md:max-w-xs pointer-events-none select-none">
                        ...
                        <p className="text-xs md:text-sm text-gray-300 mb-2">
                            ...
                            <br />
                            A compact, rectangular beam antenna with high F/B ratio.
                        </p>
