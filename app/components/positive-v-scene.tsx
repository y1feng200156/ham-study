...
export default function PositiveVAntennaScene({ isThumbnail = false }: { isThumbnail?: boolean }) {
    const [showWaves, setShowWaves] = useState(true);
    const [showPattern, setShowPattern] = useState(true);

    const LegendContent = () => (
        <>
            <h2 className="text-lg md:text-xl font-bold mb-2">正V天线 (Positive V)</h2>
            <p className="text-xs md:text-sm text-muted-foreground mb-2">
                    两端向上倾斜的偶极子天线。通常安装在桅杆顶部，以避开地面障碍物。
                <br />
                A dipole with ends tilted upwards. Usually mounted high on a mast.
            </p>
        ...
        <div className="flex flex-col gap-4">
            <div className={`relative w-full ${isThumbnail ? 'h-full' : 'h-[450px] md:h-[600px]'} border rounded-lg overflow-hidden bg-black touch-none`}>
                ...
                >
                    ...
                    {showWaves && (
                        <group position={[0, 1, 0]}>
                            <RadialWaveLines antennaType="positive-v" polarizationType="horizontal" isThumbnail={isThumbnail} />
                        </group>
                    )}
                </Canvas>
