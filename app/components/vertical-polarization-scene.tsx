...
function RadiationPattern() {
  const geometry = useMemo(() => {
    ...
    for (let i = 0; i < posAttribute.count; i++) {
        ...
        // Vertical dipole pattern: omnidirectional in XZ plane (horizontal), nulls at Y axis (vertical)
        // Gain proportional to sin(theta) where theta is angle from vertical (Y) axis
        // Simplified: gain = sqrt(x^2 + z^2) which is the horizontal component
        const gain = Math.sqrt(vertex.x * vertex.x + vertex.z * vertex.z);
...
export default function VerticalPolarizationScene({ isThumbnail = false }: { isThumbnail?: boolean }) {
    const [showWaves, setShowWaves] = useState(true);
    const [showPattern, setShowPattern] = useState(true);

    const LegendContent = () => (
        <>
            <h2 className="text-lg md:text-xl font-bold mb-2">垂直极化 (Vertical Polarization)</h2>
        ...
        <div className="flex flex-col gap-4">
            <div className={`relative w-full ${isThumbnail ? 'h-full' : 'h-[450px] md:h-[600px]'} border rounded-lg overflow-hidden bg-black touch-none`}>
                ...
                >
                    ...
                    {showWaves && <RadialWaveLines antennaType="vertical" polarizationType="vertical" isThumbnail={isThumbnail} />}
                </Canvas>
