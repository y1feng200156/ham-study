export default {
  ...
  circularPolarization: {
    ...
    about:
      "本可視化演示了<b>圓極化 (Circular Polarization)</b> 的電磁波傳播。在這種極化方式中，電場矢量隨着波的傳播而不斷旋轉，描繪出一個螺旋形狀。",
    rhcp: "<strong>右旋圓極化 (RHCP):</strong> 電場矢量沿傳播方向順時針旋轉（符合右手法則）。",
    lhcp: "<strong>左旋圓極化 (LHCP):</strong> 電場矢量沿傳播方向逆時針旋轉。",
    application:
      "<strong>應用:</strong> 圓極化廣泛用於<b>衛星通信</b>，因為它可以抵抗法拉第旋轉（信號穿過電離層時極化面的改變），並且不要求接收天線與發射天線的極化角度精確對齊（不需要像線極化那樣嚴格水平或垂直對齊）。",
    ...
    physicsContent:
      "對於螺旋天線 (Helical Antenna)，當螺旋周長接近一個波長時，天線工作在<strong>軸向模式 (Axial Mode)</strong>。此時，天線沿螺旋軸向輻射最強，形成特定的方向性波束 (Directional Beam)，且產生近似完美的圓極化波。本演示中的輻射方向圖正是基於此模式繪製，而非側向模式 (Normal Mode) 的全向輻射。",
    physicsQuote:
  ...
  verticalPolarization: {
    ...
    crossPolNote:
      "這也是為甚麼這兩種極化方式可以復用同一頻率而不容易互相干擾。",
    vToC: "<strong>垂直發射 -> 圓極化接收 (Vertical to Circular):</strong>",
    vToCNote:
      "線性極化波可以分解為兩個相反旋轉的圓極化波，接收天線只能接收其中一個分量，因此損失一半能量 (3dB)。儘管有損耗，但考慮到極化失配的風險，這種組合在特定情況下（如移動通信）是可以接受的。",
    physicsContent:
      "根據天線理論，垂直偶極子 (Vertical Dipole) 產生的電場只有垂直分量。其在水平面上的輻射強度是均勻的，形成全向方向圖。這意味着'電場矢量'(Vertical) 與 '主輻射波瓣方向'(Horizontal Plane) 在幾何上確實是正交的。",
    physicsQuote:
  ...
  horizontalPolarization: {
    ...
    hToVNote:
      "巨大的信號損耗 (約 <strong>-20dB 到 -30dB</strong>)。在短波 DX (遠距離通信) 中，由於電離層反射經常改變極化方向，這種影響可能不如視距通信(VHF/UHF)那麼顯著，但在視距通信中是致命的。",
    physicsContent:
      "水平極化偶極子天線 (Horizontal Dipole) 產生的電場矢量平行於地面。其輻射方向圖在自由空間中是圍繞導線的圓環 (doughnut)，但受地面反射影響，實際輻射圖通常呈現為從地面向上翹起的瓣狀。水平極化在HF波段 DX 通信中非常流行，部分原因是它比垂直極化受地面噪聲干擾更小。",
    physicsQuote:
  ...
  dipoleAntenna: {
    ...
    overview:
      "本頁面提供了<strong>偶極子天線</strong>的交互式 3D 仿真。你可以通過調整天線長度（0.5λ, 1.0λ, 1.5λ）來觀察駐波分布的變化，也可以在“倒V”模式下直觀地看到天線變形對輻射特性的影響。綠色網格球體顯示了理想狀態下的輻射方向圖。",
    structure:
      "<strong>結構仿真</strong>：場景中紅藍兩色的振子代表偶極子的兩臂，中間白色為饋電點。黃色線條動態展示了電流駐波的分布。",
    halfWave:
    ...
    principle:
      "仿真中的黃色曲線展示了電流駐波的分布。注意觀察電流波腹（最大值）和波節（零點）的位置變化。",
    principleDetails: {
    ...
    patternIntro:
      "綠色的 3D 網格可視化了由以下公式計算出的遠場輻射強度 <M>F(\\theta)</M>。注意它是如何隨着天線長度 <M>L</M> 的變化而“呼吸”的。",
    halfWaveSpecialCase:
      "半波偶極子特例 (<M>L=\\lambda/2 \\Rightarrow kL/2 = \\pi/2</M>)",
    fullWaveTitle: "為甚麼不用全波長偶極子？",
    fullWaveIntro: "如果你在模擬器中將長度設為 <M>1.0\\lambda</M>，你會發現：",
    fullWavePoints: {
      pattern:
        "<strong>方向圖</strong>：雖然變窄了一點，但依然是 8 字形，增益略有提升。",
      impedance:
        "<strong>阻抗災難</strong>：在中心點，全波長天線的電流為 0（波節），電壓極高。這意味着輸入阻抗 <strong>Z 趨近於無窮大</strong>（數千歐姆）。",
      conclusion:
    ...
    physicsContent:
      "細偶極子天線上的電流分布近似為正弦分布，兩端為零點。這種駐波分布產生了向外輻射的電磁場。",
    physicsQuote:
  ...
  gpAntenna: {
    ...
    theoryAnalysis: "理論分析",
    theoryContent:
      "GP天線（Ground Plane）利用<strong>鏡像原理</strong>，通過地網在地下產生一個虛擬的鏡像振子，從而構成完整的垂直偶極子。實體的 1/4 波長振子與虛擬的下半部分相結合，實現了全尺寸天線的效率。通過調整地網下垂角度至約 135°，可以提高輸入阻抗至 <strong>50Ω</strong>，完美匹配同軸電纜。其低仰角輻射特性使其成為 DX 通信的利器。",
  ...
  invertedVAntenna: {
    ...
    about:
      "倒V天線實際上就是中間架高、兩端下垂的偶極子天線 (Dipole)。由於其架設簡單（只需一根支撐杆），是業餘無線電愛好者最常用的短波天線之一。",
    theoryAnalysis: "理論分析",
    theoryContent:
      "倒V天線（Inverted V）是“窮人的法拉利”。它的核心優勢在於<strong>架設極簡</strong>和<strong>天然阻抗匹配</strong>。只需一根中心支撐杆，利用重力自然下垂。當兩臂夾角在 90° 到 120° 之間時，輸入阻抗會從平拉偶極子的 73Ω 自然降低到約 <strong>50Ω</strong>，無需任何阻抗變換器即可直接連接同軸電纜。此外，其垂直極化分量增強了全向輻射特性，非常適合近距離 NVIS 通信。",
    ...
    physicsContent:
      "倒V天線 (Inverted-V) 的兩臂下垂會影響其輻射阻抗和方向圖。隨着夾角小於 180°，輸入阻抗會降低（通常降至 50Ω 附近），使其能直接匹配同軸電纜。此外，垂直輻射分量會有所增加，填充了水平偶極子兩側的零點，使方向圖在全方位上更均勻。",
    physicsQuote:
  ...
  moxonAntenna: {
    ...
    physicsContent:
      "Moxon 矩形天線利用了'臨界耦合'原理。通過將元件末端向內彎曲並保持特定的間距，使得有源振子與反射器之間的互感耦合增強。這種特定的幾何結構產生了近乎完美的心形 (Cardioid) 輻射方向圖，在保持前向增益的同時，極大程度地消除了後向輻射 (極高的 F/B 比)。",
    physicsQuote:
  ...
  positiveVAntenna: {
    ...
    rotatable:
      '<strong>旋轉偶極子:</strong> 在短波波段，正V結構常用於製作"旋轉偶極子" (Rotatable Dipole)。V形結構減小了迴轉半徑，使得天線更緊湊，轉動慣量更小。',
  ...
  quadAntenna: {
    ...
    metaDescription:
      "3D演示方框天線（Quad Antenna）的迴路結構，展示其高增益和低輻射仰角的特性。",
    metaKeywords:
      "方框天線, Quad antenna, 定向天線, directional antenna, DX利器, 高增益, high gain",
    title: "方框天線 (Quad Antenna)",
    about:
      "方框天線（通常指 Cubical Quad）由兩個或多個方形迴路元件組成。由 Clarence Moore (W9LZX) 在 1940 年代為解決高海拔電暈放電問題而發明。",
    highGain:
      "<strong>高增益:</strong> 2 單元的 Quad 天線通常相當於 3 單元的八木天線增益。",
    lowNoise:
      "<strong>低噪音:</strong> 閉合迴路結構有助於減少接收時的靜電噪音，聽感通常比八木更安靜。",
    theoryAnalysis: "理論分析",
    theoryContent:
      "全波諧振環（Full-Wave Loop）本質上是一個<strong>變形的雙偶極子陣列</strong>。當環周長約為 1 個波長 (<M>\\lambda</M>) 時，環上的電流分布形成駐波，最大輻射方向垂直於框面（Broadside）。這種閉合迴路結構不僅提供了比半波偶極子更高的增益（約 3.3 dBi），還因為它是直流接地的，能有效抑制降水靜電噪音，使接收背景更安靜。",
    ...
    physicsContent:
      "四方框天線 (Quad) 是一個全波長閉合迴路，相比於半波長偶極子，它具有更有效的輻射孔徑。標準的 2 單元 Quad (有源振子 + 反射器) 通過臨界耦合可提供約 7dBi 的增益，相當於 3 單元八木。由於它是直流接地的閉合迴路，能有效洩放雨雪靜電 (Precipitation Static)，因此接收背景噪音通常比八木更低。",
    physicsQuote:
  ...
  yagiAntenna: {
    ...
    physicsContent:
      "八木天線的定向性源於寄生單元（反射器和引向器）上的感應電流與有源振子電流之間的<strong>相位差 (Phase Difference)</strong>。反射器通常比諧振長度略長 (呈感性)，電流滯後；引向器略短 (呈容性)，電流超前。這種相位關係導致信號在前方疊加增強，在後方抵消。本演示正是通過在各單元上設置相應的相位偏移來模擬這一物理現象。",
    physicsQuote:
  ...
  endFedAntenna: {
    ...
    structure:
      "<strong>結構:</strong> 主要由一個 Unun 盒子、一根長振子線（Radiator）和一段同軸電纜組成。同軸電纜的屏蔽層通常充當反向地（Counterpoise）。",
    multiband:
      "<strong>多波段:</strong> 在基頻的諧波頻率上也能自然諧振，無需天調即可工作在多個波段（如 40m, 20m, 15m, 10m）。",
    polarizationTitle: "極化與應用",
    physicsContent:
      "端饋半波天線 (EFHW) 在諧振時，饋電點位於電壓波腹（電壓最大）和電流波節（電流最小）處。根據 Z=V/I，這意味着其輸入阻抗極高（理論上無窮大，實際上約 2500-5000Ω）。因此必須使用高變比（如 49:1 或 64:1）的寬帶阻抗變換器將其降至 50Ω。",
    physicsQuote:
    ...
    patternDesc:
      "隨着諧波次數 <M>n</M> 的增加，原本單一的波瓣會分裂成多個花瓣，且最大輻射方向會越來越貼近導線軸線。",
    commonMode:
  ...
  longWireAntenna: {
    ...
    about:
      "長線天線是指長度至少為一個波長（通常更長）的天線。它不同於半波偶極子，其輻射方向圖會隨着長度的增加而變得複雜。",
    gain: "<strong>增益與方向性:</strong> 隨着天線長度的增加（以波長為單位），主波束會越來越貼近導線方向，且增益會增加。",
    lobes:
      "<strong>多瓣特性:</strong> 長線天線會產生多個輻射瓣。長度越長，瓣數越多。",
    ground:
      "<strong>接地系統:</strong> 對於非諧振的長線（如 Random Wire），良好的接地是必不可少的，它是天線系統的一部分。",
    physicsContent:
      "長線天線上的電流分布通常被視為駐波（若末端開路）或行波（若末端終止）。對於末端開路的長線，其輻射方向圖是典型的多瓣狀。",
    physicsQuote:
    ...
    theoryDesc:
      "長線天線，特別是在多個波長下工作時，會表現出由沿導線的駐波分布決定的複雜輻射圖。",
    theoryFormulaIntro:
  ...
  windomAntenna: {
    ...
    metaDescription:
      "3D演示溫頓天線(OCFD)的偏饋結構、匹配原理及多波段工作特性。",
    metaKeywords:
      "溫頓天線, Windom antenna, OCFD, 偏饋偶極子, Off-Center Fed Dipole, 4:1 balun, 多波段天線",
    title: "溫頓天線 (Windom / OCFD)",
    overviewTitle: "關於此演示 (About this Demo)",
    overview:
      "本頁面演示了<strong>溫頓天線 (Windom)</strong>，也稱為<strong>偏饋偶極子 (OCFD)</strong>。通過將饋電點從中心移開，我們在不損失太多效率的情況下獲得了多波段工作的能力。場景中的不對稱結構正是其核心特徵。",
    structure:
      "<strong>結構仿真</strong>：饋電點（白色盒子）位於天線總長度的約 <strong>33%</strong> 處。這種“偏心”設計徹底改變了天線的阻抗分布。",
    principleTitle: "為甚麼是 33% 處？ (The Magic Point)",
    principleIntro:
      "如果我們觀察不同頻率下的電流駐波分布（黃色曲線），會發現 1/3 處是一個神奇的位置：",
    principlePoints: {
    ...
    principleConclusion:
      "<strong>結論</strong>：僅需一個饋電點，就能在 40m, 20m, 10m 等偶數倍頻波段獲得相似的阻抗。",
    ...
    polarizationException:
      "只有在<strong>共模干擾</strong>嚴重時（巴倫失效，饋線輻射），垂直的饋線波與水平的天線波混合，才可能導致極化狀態混亂。但這屬於<strong>信號畸變</strong>，而非設計特性。",
    physicsContent:
      "溫頓天線利用了導線上特定位置（約離中心17%處，即全長33%處）在多個諧波頻率下阻抗近似相等的特性。這使得單一饋電點配合4:1巴倫即可實現多波段工作。",
    physicsQuote:
  ...
  magneticLoopAntenna: {
    ...
    features:
      "<strong>特點:</strong> 效率較低，但具有極高的 Q 值和極窄的帶寬。最著名的特點是其卓越的抗干擾能力。",
    physicsModelTitle: "物理模型：磁偶極子",
    physicsModel:
      "小環天線在物理上等效於一個<strong>磁偶極子</strong>。由於尺寸遠小於波長，假設環上的電流 <M>I</M> 是均勻分布的（這與偶極子天線不同）。",
    ...
    physicsContent:
      "磁環天線在近場主要輻射磁場分量。在遠場，其電場分量是水平的（<M>\\phi</M> 方向），其輻射方向圖呈冬甩狀，零點位於軸線上。",
    physicsQuote:
